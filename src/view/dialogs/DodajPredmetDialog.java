...
/**
 * Klasa predstavlja dijalog dodavanja novog predmeta u informacioni sistem.
 * Sastoji se od tekstualnih polja u koja se unose informacije o predmetu, njima
 * korespondentnih labela i dugmeta za potvrdu i odustanak dodavanja novog
 * predmeta. Za svako tekstualno polje su vezani slušači događaja koji svaki put
 ...
 * ispravno uneo tu informaciju o predmetu. Za dugme potvrde dodavanja novog
 * predmeta je takođe vezan slušač događaja u kom se nalaze dodatne provere.
 * Kada se klikne dugme potvrde, na primer ako predmet sa istom šifrom već
 * postoji u sistemu pojaviće se novi dijalog koji daje do znanja referentu da
 * je to slučaj i da treba da promeni šifru predmeta koji hoće da doda. Ako je
 * sve uneto ispravno, kreira se objekat novog predmeta. Ovaj objekat se dodaje
 * u listu svih predmeta baze predmeta preko kontrolera predmeta.
 * 
 * @author Luka Miletić
 *
 */
public class DodajPredmetDialog extends JDialog {
	...
	/**
	 * Dugme potvrde dodavanja novog predmeta.
	 */
	private JButton ok;

	/**
	 * Kreira dijalog dodavanja novog predmeta, centriran u odnosu
	 * na glavni prozor.
	 * 
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (Predmet p : PredmetController.getInstance().getPredmeti()) {
					if (p.getSifra().equals(sifraVrednost)) {
						JOptionPane.showMessageDialog(getContentPane(), "Već postoji predmet " + "sa ovom šifrom!");
						return;
					}
				}

				Predmet predmet = new Predmet(sifraVrednost, nazivVrednost, Integer.parseInt(ESPBVrednost),
						godinaStudija, semestarVrednost);
	...
	/**
	 * Omogućuje prosleđeno dugme ako je pokazatelj celokupne
	 * ispravnosti unosa <code>true</code>. U suprotnom ga
	 * onemogućuje.
	 * 
	 * @param button dugme koje omogućujemo/onemogućujemo
	 */
